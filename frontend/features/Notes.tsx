...
import { Plus, Search, Tag, FileText, Trash2, ArrowLeft } from 'lucide-react';
...
const Notes: React.FC<NotesProps> = ({ notes, addNote, updateNote, updateNoteTitle, deleteNote }) => {
  ...
  // Mobile: show editor full screen when note is selected
  const showMobileEditor = isMobile && selectedNoteId && activeNote;
  const showNoteList = !isMobile || !selectedNoteId;

  return (
    <div className="h-full flex flex-col gap-4 md:gap-6 animate-fade-in max-h-[calc(100vh-6rem)]">
      {/* Title — hidden when mobile editor is open */}
      {!showMobileEditor && (
        <h2 className="text-xl md:text-2xl font-bold text-sys-text-main dark:text-dark-text tracking-tight shrink-0 pl-2">Notas</h2>
      )}

      <div className="flex-1 flex gap-8 overflow-hidden">

        {/* Sidebar List — visible on desktop always, on mobile only when no note selected */}
        {showNoteList && (
          <div className={`flex flex-col bg-sys-card dark:bg-dark-card rounded-3xl border border-sys-border dark:border-dark-border shadow-soft overflow-hidden ${isMobile ? 'w-full' : 'w-80'}`}>
            <div className="p-4 md:p-6 border-b border-sys-border dark:border-dark-border">
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 opacity-100 md:opacity-0 md:group-hover:opacity-100 text-sys-text-sub hover:text-calm-coral transition-all p-1"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
        ...
        {/* Editor — on mobile shows full screen with back button when note selected */}
        {(!isMobile || showMobileEditor) && (
          <div className={`bg-sys-card dark:bg-dark-card rounded-3xl border border-sys-border dark:border-dark-border shadow-soft overflow-hidden flex flex-col relative ${isMobile ? 'w-full' : 'flex-1'}`}>
            {/* Soft Background Accent */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-soft-lilac/10 rounded-full blur-3xl -z-0 pointer-events-none"></div>

            {activeNote ? (
              <>
                <div className="p-4 md:p-8 md:pb-4 border-b border-sys-border dark:border-dark-border flex items-center gap-3 z-10">
                  {/* Mobile back button */}
                  {isMobile && (
                    <button
                      onClick={handleBackToList}
                      className="shrink-0 p-2 -ml-1 text-sys-text-sub hover:text-sys-text-main dark:hover:text-dark-text transition-colors rounded-lg"
                    >
                      <ArrowLeft size={22} />
                    </button>
                  )}
                  <input
                    ...
                    className="text-2xl md:text-3xl font-bold text-sys-text-main dark:text-dark-text outline-none bg-transparent w-full"
                  />
                </div>
                <div className={`px-4 md:px-8 py-2 flex items-center gap-2 text-xs uppercase tracking-wide font-bold z-10 ${NOTE_CATEGORY_COLORS[activeNote.category]}`}>
                  <Tag size={14} />
                ...
                <textarea
                  ...
                  className="flex-1 w-full p-4 md:p-8 pt-4 md:pt-6 outline-none resize-none text-sys-text-main dark:text-dark-text leading-relaxed bg-transparent z-10 custom-scrollbar text-base"
                />
                <div className="p-3 md:p-4 border-t border-sys-border dark:border-dark-border text-right text-xs text-sys-text-sub z-10 font-medium bg-sys-card dark:bg-dark-card">
